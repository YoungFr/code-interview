...
1. 监控：多个哨兵一起监控主节点，一个哨兵认为下线时会向其他哨兵拉票，赞同票数达到 quorum 后认为下线。
2. 选主：判断主节点下线的哨兵成为候选者，向其他哨兵拉票，希望选举自己来进行主从故障转移。
3. 转移：选出的哨兵领导节点进行以下操作
   1. 在已下线主节点的从节点中选出一个作为新主节点。
   2. 让其他从节点修改复制目标为这个新主节点。
   3. 将新主节点的信息通过发布/订阅模式通知给客户端。

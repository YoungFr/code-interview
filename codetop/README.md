...
1. [三数之和](../algos/tp/tp6.go) 排序 + 双指针 + 元素去重
...
1. [缺失的第一个正数](../algos/array/array17.go) 原地哈希 `a[i] = i+1` => 遍历数组如果 `a[i]` 在 `[1, n]` 之间则把它和 `a[a[i]-1]` 交换
...
1. [最长重复子数组/最长公共子串](./problems/c083_lcsubstring.go) 注意和最长公共子序列的区别：只有相等时才能转移
...
1. 移掉 k 位数字 402 单调栈 => 对于遍历到的每个数字，如果它小于栈顶元素则把栈顶弹出

1. 盛最多水的容器 11

1. [二叉搜索树与双向链表](https://leetcode.cn/problems/er-cha-sou-suo-shu-yu-shuang-xiang-lian-biao-lcof/description/)
...
1. [排序奇升偶降链表](https://mp.weixin.qq.com/s/0WVa2wIAeG0nYnVndZiEXQ)
...
1. [青蛙跳台阶问题](../algos/dp/dp81.go) 同第 47 题

1. [连续子数组的最大和](../algos/array/array13.go) 同第 7 题
